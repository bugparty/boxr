...
// </RTEN>
        ...
        if (glew_err != GLEW_OK) {
            spdlog::get(name)->error("[timewarp_gl] GLEW Error: {}", (const char*)glewGetErrorString(glew_err));
...
// </RTEN>
        ...
        // std::this_thread::sleep_for(std::chrono::milliseconds(19)); // <RTEN> manual sleep to control load
